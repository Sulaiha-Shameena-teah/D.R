...
/*const daily_work_details = {
	...
	'data-accessors':{'id':'customer_id','name':'customer_name','emi':'emi','pending':'pending','bal':'balance','status':'status','date':'collection_date','rec_amt':'received_amount','agent_id':'agent_id','current_balance':'current_balance'}	
};*/
	...
	$(document).on('click','.saveButton, .removeButton',function(e){
		if($("#page_type").val() == daily_work_details['page-value'] || $("#page_type").val() == admin_work_details['page-value'] || $("#page_type").val() == admin_approval_details['page-value']){
			...
			if($("#page_type").val() == daily_work_details['page-value']){
					...
					post_endpoint = admin_work_details['store-admin-work-endpoint'];
					if(status == 'Received'){
		...
		if(total_count > 0 || checkValid(search_val)){
			...
			if(data['dataList']){
				$.each(dataList, function(index,val){
					...
					'<td><button type="button" class="fa fa-angle-double-down saveButton" style="font-size:36px"><span class="material-icons"></span></button><input class="agent_id" type="hidden" value="'+val[daily_work_details['data-accessors']['agent_id']]+'"/><input type="hidden" class="collection_date" value="'+val[daily_work_details['data-accessors']['collection_date']]+'"/></td>'+
					'</tr>';
	...
	url: base_url+investor_report_endpoint+"?limit="+limit+"&offset="+offset+"&query="+search_val
		...
		if(total_count > 0){
			$.each(data['dailyWorksList'], function(index,val){
				...
				var append_table = '<tr class="row_'+index+'"><td>'+(index+1)+'</td><td class="agent_id">'+(val[investor_details['data-accessors']['agent_id']]!=undefined ? val[investor_details['data-accessors']['agent_id']] : "")+'</td><td>'+(val[investor_details['data-accessors']['name']]!=undefined ? val[investor_details['data-accessors']['name']] : "")+'</td><td class="phone">'+(val[investor_details['data-accessors']['phone']]!=undefined ? val[investor_details['data-accessors']['phone']] : "")+'</td><td class="addr">'+(val[investor_details['data-accessors']['addr']]!=undefined ? val[investor_details['data-accessors']['addr']] : "")+'</td><td class="aadhar">'+(val[investor_details['data-accessors']['aadhar']]!=undefined ? val[investor_details['data-accessors']['aadhar']] : "")+'</td><td class="pan">'+(val[investor_details['data-accessors']['pan']]!=undefined ? val[investor_details['data-accessors']['pan']] : "")+'</td>';
				$("#electric_table").append(append_table);
			});
			if(total_count >10){
				renderPagination(total_count,page_no,10,1);
	...
	url: endpoint
		...
		if(total_count > 0 || checkValid(search_val)){
			...
			if(data['dataList']){
				$.each(dataList, function(index,val){
					// calculation =  Previous + Current - Received = Balance.
					var rec_amt = val[admin_work_details['data-accessors']['rec_amt']]!=undefined ? val[admin_work_details['data-accessors']['rec_amt']] : 0;
					var previous = val[admin_work_details['data-accessors']['pending']]!=undefined ? val[admin_work_details['data-accessors']['pending']] : 0;
					var current = val[admin_work_details['data-accessors']['current_balance']]!=undefined ? val[admin_work_details['data-accessors']['current_balance']] : 0;
					var emi = val[admin_work_details['data-accessors']['emi']]!=undefined ? val[admin_work_details['data-accessors']['emi']] : 0;
					var bal = (parseInt(previous) + parseInt(current)) - parseInt(rec_amt);
					if(bal<0){
						bal = 0;
					}
